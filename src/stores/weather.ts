...
export const useWeatherStore = defineStore('weather', () => {
  const geolocation = reactive({
    ...
    lastCoords: useStorage('geolocation-last-coords', {} as GeolocationCoords),
  });
  ...
  const errorMessage = computed(() => {
    if (weather.hasFetchingError) {
      return 'Sorry, the weather service is currently unavailable';
    } else if (geolocation.search.hasFetchingError) {
      return 'Sorry, the search service is currently unavailable';
    } else if (geolocation.api.type === 'error') {
      if (!geolocation.search.hasFetchingError) {
  ...
  const pickNextHourlyWeather = computed(
    (): (
      ...
      const sortArrayByDate = (
        ...
        sunrise: (typeof weather.info.weather)[number]['astronomy']['sunrise'],
        sunset: (typeof weather.info.weather)[number]['astronomy']['sunset'],
      ) => {
        return [
      ...
      let hourlyWeatherWithSunTimes = sortArrayByDate(
        weather.info.weather[0]!.hourly,
        weather.info.weather[0]!.astronomy.sunrise,
        weather.info.weather[0]!.astronomy.sunset,
      );
      ...
      hourlyWeatherWithSunTimes = sortArrayByDate(
        weather.info.weather[1]!.hourly,
        weather.info.weather[1]!.astronomy.sunrise,
        weather.info.weather[1]!.astronomy.sunset,
      );
  ...
  async function getGeolocationViaSearch(
    ...
    if (!searchParam) {
      geolocation.search.data = [];
      return;
    }
    try {
      geolocation.search.isFetching = true;
      ...
      geolocation.search.hasFetchingError = false;
      geolocation.search.data = res.map((geolocation) => {
      ...
      geolocation.search.isFetching = false;
    }
  }
  async function getWeather(coords?: AdjSearchApiResponse | GeolocationCoords) {
    ...
    try {
      ...
      weather.hasFetchingError = false;
